...
from tools._tensorrt_bootstrap import prefer_sdk_tensorrt
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='evaluation metric(s); defaults to config value when set, '
             'otherwise falls back to "miou" for occupancy models')
...
def resolve_eval_metrics(args, cfg):
    metric = args.metric
    if metric is None:
        metric = cfg.get('evaluation', {}).get('metric')
    if metric is None and cfg.model.get('wocc', False):
        metric = ['miou']
    if metric is None:
        raise ValueError(
            'Evaluation metric is not specified. Pass --metric explicitly or '
            'set evaluation.metric in the config.')
...
def main():
    ...
    eval_kwargs.update(dict(metric=resolve_eval_metrics(args, cfg)))
