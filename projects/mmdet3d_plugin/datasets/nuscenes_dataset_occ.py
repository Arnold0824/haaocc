...
class NuScenesDatasetOccpancy(NuScenesDataset):
    ...
    def _normalize_metric(metric):
        if isinstance(metric, (list, tuple)):
            if len(metric) == 0:
                raise ValueError('Evaluation metric list cannot be empty.')
            return metric[0]
        if isinstance(metric, bool) or metric is None:
            return 'miou'
        return metric
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
        metric = self._normalize_metric(eval_kwargs.get('metric'))
        print("metric = ", metric)
        if metric == 'ray-iou':
            occ_gts = []
            occ_preds = []
