...
class BEVOCCHead2D(BaseModule):
    ...
    def loss(self, occ_pred, voxel_semantics, mask_camera):
        ...
        if self.use_mask:
            ...
            voxel_semantics_ce = voxel_semantics.reshape(-1)
            # (B, Dx, Dy, Dz, n_cls) --> (B*Dx*Dy*Dz, n_cls)
            preds_ce = occ_pred.reshape(-1, self.num_classes)
            # (B, Dx, Dy, Dz) --> (B*Dx*Dy*Dz, )
            mask_camera_ce = mask_camera.reshape(-1)
            # voxel_semantics[mask_camera.bool()] = 255
            if self.class_balance:
                valid_voxels = voxel_semantics[mask_camera.bool()]
                num_total_samples = 0
                for i in range(self.num_classes):
            ...
            loss['loss_occ'] = loss_occ
            loss['loss_voxel_sem_scal'] = sem_scal_loss(preds, voxel_semantics) * 0.2
            loss['loss_voxel_geo_scal'] = geo_scal_loss(preds, voxel_semantics, non_empty_idx=17) * 0.2
            loss['loss_voxel_lovasz'] = lovasz_softmax(torch.softmax(preds, dim=1), voxel_semantics) * 0.5

            loss_ce = self.loss_ce(
                preds_ce,      # (B*Dx*Dy*Dz, n_cls)
                voxel_semantics_ce,    # (B*Dx*Dy*Dz, )
                mask_camera_ce,        # (B*Dx*Dy*Dz, )
                avg_factor=num_total_samples
            )
...
class BEVOCCHead2D_HAA(BEVOCCHead2D):
    def __init__(self, use_haa=True, haa_mode='feature_gate', **kwargs):
        ...
        if self.use_haa:
            ...
            self.haa = nn.Sequential(
                nn.Conv2d(self.in_dim, mid_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, img_feats):
        ...
        if self.use_predicter:
            ...
            occ_logits = self.predicter(occ_feats.permute(0, 3, 2, 1).contiguous())  # (B, Dx, Dy, Dz*n_cls)
            bs, dx, dy, _ = occ_logits.shape
            occ_voxels = occ_logits.view(bs, dx, dy, self.Dz, self.num_classes)       # (B, Dx, Dy, Dz, n_cls)
            occ_voxels = occ_voxels.permute(0, 4, 3, 2, 1).contiguous()                # (B, n_cls, Dz, Dy, Dx)
        else:
            # out_channels already equals Dz*n_cls
            bs, _, dy, dx = occ_feats.shape
        ...
        if self.use_haa and self.haa_mode == 'legacy_logit_scale':
            ...
            if attn.shape[-3:] != occ_voxels.shape[-3:]:
                raise RuntimeError(
                    f"HAA shape mismatch: attn={attn.shape}, voxels={occ_voxels.shape}"
                )
            # 【核心修改点】使用 gamma 控制 HAA 的介入程度
            occ_voxels = self.gamma *(occ_voxels * attn) + occ_voxels

        # Keep base loss/get_occ compatibility: (B, Dx, Dy, Dz, n_cls)
        occ_pred = occ_voxels.permute(0, 4, 3, 2, 1).contiguous()
        return occ_pred
    ...
    def loss(self, occ_pred, voxel_semantics, mask_camera):
        ...
        if self.use_mask:
            ...
            if self.class_balance:
                ...
                for i in range(self.num_classes):
                    num_total_samples += (valid_voxels == i).sum() * self.cls_weights[i]
                cls_weight = self.cls_weights.to(preds)
            else:
                num_total_samples = mask_camera.sum()
                cls_weight = preds.new_ones(self.num_classes)
            ...
            loss['loss_occ'] = loss_occ * 100
            loss['loss_voxel_sem_scal'] = sem_scal_loss(preds, voxel_semantics) * 0.2
            loss['loss_voxel_geo_scal'] = geo_scal_loss(preds, voxel_semantics, non_empty_idx=17) * 0.2
            loss['loss_voxel_lovasz'] = lovasz_softmax(torch.softmax(preds, dim=1), voxel_semantics) * 0.5

            if hasattr(self, 'loss_ce') and self.loss_ce is not None:
                loss_ce = self.loss_ce(
                    preds_ce,
                    voxel_semantics_ce,
                    mask_camera_ce,
